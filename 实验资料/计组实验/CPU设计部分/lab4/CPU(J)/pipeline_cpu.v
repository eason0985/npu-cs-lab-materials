module pipeline_cpu(input clock, //时钟信号
	...
	parameter mux_s_num_write_31 = 5'd31;
	parameter beq_addr = 5'd31;
	wire [31:0] pc;//当前pc
	...
	wire id_exe_flush;
	wire [31:0] abs_addr;
	wire [25:0] instr_index = instruction_out[25:0];
	wire [1:0] s_npc;
	wire if_id_flush;
	wire [31:0] pcadd4;
	...
	mux_32_3to1 MUX_S_FORWARDA3(
		.s(s_forwardA3),
		.zero(result_data_write),
		.one(c_out),
		.two(a_out),
		.result(result_forwardA3)
		);
		
	mux_32_3to1 MUX_S_FORWARDB3(
		.s(s_forwardB3),
		.zero(result_data_write),
		.one(c_out),
		.two(b_out_1),
	...
	blockage BLOCKAGE(
		...
		.if_id_write(if_id_write),
		.id_exe_flush(id_exe_flush),
		.instruction_id(instruction_out),
		.instruction_exe(instruction_1),
		.if_id_flush(if_id_flush)
		);
	...
	bypass BYPASS(
		...
		.num_write_wb(num_write_3),
		.reg_write_wb(reg_write_3)
		);
